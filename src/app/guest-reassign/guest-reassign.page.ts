...
@Component({
  selector: 'app-guest-reassign',
  templateUrl: './guest-reassign.page.html',
  styleUrls: ['./guest-reassign.page.scss'],
})
export class GuestReassignPage implements OnInit {
	...
	scheduleData(){
		...
		this.globleServiceService.GlobalHit(key,'guest_work_allocation').
		then(data=>{
			this.spinner.hide()
			...
			this.workerData.minute=data['work_allocation'].data_list[0].total_service_time
	...
	reassignWorker(){
		...
		if(this.workerData.reassign_worker=='0' || this.workerData.reassign_worker==null || this.workerData.reassign_worker==undefined){
			...
			this.globleServiceService.GlobalHit(key,'guest_reassign_worker').
			then(data=>{
				this.spinner.hide()
				if(data['responseCode']==200){
					...
					this.storage.get('valueObj').then((val) => {
						...
						if(localValue==undefined || localValue==null || localValue==''){
							...
							this.router.navigateByUrl('/guest-schedule-management/'+ localValue.workerId +'/'+ localValue.job_date +'/'+ localValue.workerName )
							this.storage.remove('valueObj')
						}
					});
				}else{
